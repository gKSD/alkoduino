#include <avr/io.h> //дополнительно инклудится в Arduino.h
#include <avr/interrupt.h> //дополнительно инклудится в Arduino.h
...
namespace {
    ...
    void _setMotorDirections( bool motor1_dir, bool motor2_dir)
    {
        bool direction = motor1_dir;
        if (direction ^ _motorConnection_1)    
            PORTD |= (1 << 4);//_BV(4);  //digitalWrite(4, HIGH)
        else
            PORTD &= ~(1 << 4);//~_BV(4); //digitalWrite(4, LOW)
        
        direction = motor2_dir;
